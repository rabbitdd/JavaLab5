...
import main.Mood;
import main.WeaponType;

import java.time.LocalDate;
import java.util.HashSet;
import java.util.Scanner;
/**
 * Класс filler (Используется для заполнения элеметов колекции)
...
public class Filler {
    ...
    Filler(HashSet<Long> ID) {
        this.ID = ID;
    }

    public Long fillId(Long n) {
        Long id = (long)(Math.random() * n) + 1;
        if (ID.isEmpty() || !ID.contains(id)) {
            ID.add(id);
            return id;
        }
        else {
